...
from numpy import apply_along_axis, vstack, select
...
class cnrps_pension_servie(Variable):
    ...
    def formula(individu, period, parameters):
        annee_de_liquidation = (
            individu("cnrps_liquidation_date", period)
            .astype("datetime64[Y]")
            .astype(int)
            + 1970
        )
        ...
        pension_au_31_decembre_annee_precedente = individu(
            "cnrps_pension_au_31_decembre", last_year
        )
...
class cnrps_salaire_de_reference(Variable):
    ...
    def formula(individu, period):
        ...
        moyenne_2_salaires_plus_eleves = apply_along_axis(
            ...
            arr=vstack(
                [
                    individu("cnrps_salaire_de_base", period=year, options=[ADD])
                    for year in range(period.start.year, period.start.year - n, -1)
                ]
            ),
        )
        derniere_remuneration = individu(
            "cnrps_salaire_de_base", period=period, options=[ADD]
        )
        salaire_refererence = where(
            individu("cnrps_salaire_de_reference_calcule_sur_demande", period),

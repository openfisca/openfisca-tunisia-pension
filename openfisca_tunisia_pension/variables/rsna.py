...
from openfisca_core.model_api import apply_thresholds, ADD, Enum
...
class rsna_RSNATypesRaisonDepartAnticipe(Enum):
    ...
    licenciement_economique = (
        "Licenciement économique avec au minimum 60 mois de cotisations (20 trimestres)"
    )
...
class rsna_pension_minimale(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        return apply_thresholds(
            duree_assurance / 4,
            [rsna.stage_derog, rsna.stage_requis],
            [0, pension_minimale.inf * smig_annuel, pension_minimale.sup * smig_annuel],
        )
...
class rsna_pension_servie(Variable):
    ...
    def formula(individu, period, parameters):
        annee_de_liquidation = (
            individu("rsna_liquidation_date", period)
            .astype("datetime64[Y]")
            .astype(int)
            + 1970
        )
        ...
        pension_au_31_decembre_annee_precedente = individu(
            "rsna_pension_au_31_decembre", last_year
        )
...
class rsna_salaire_de_reference(Variable):
    ...
    def formula(individu, period):
        ...
        salaire_refererence = apply_along_axis(
            ...
            arr=vstack(
                [
                    individu("rsna_salaire_de_base", period=year, options=[ADD])
